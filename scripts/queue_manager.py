...
class QueueManager:
    def __init__(self, status_file="logs/queue_status.json"):
    ...
    def _save_state(self):
        ...
        data = {"updated_at": datetime.now().isoformat(), "jobs": [j.to_dict() for j in self.jobs.values()], "stats": self.get_stats()}
        with open(self.status_file, "w") as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
    def add_job(self, filename, input_path, output_path, priority=2):
        ts = datetime.now().strftime("%Y%m%d%H%M%S")
        job_id = ts + "_" + filename
        job = Job(id=job_id, filename=filename, input_path=input_path, output_path=output_path, status=JobStatus.PENDING, priority=priority, created_at=datetime.now().isoformat())
    ...
    def start_job(self, job_id):
        if job_id in self.jobs:
            ...
            self._save_state()
    def complete_job(self, job_id, cost_usd=0.0):
        if job_id in self.jobs:
    ...
    def scan_input_dir(self, input_dir, output_dir):
        new_jobs = []
...
def create_queue_manager_from_config(config):
